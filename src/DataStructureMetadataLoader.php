...
class DataStructureMetadataLoader
{
    /** @var string */
    protected $path;
    /** @var string */
    ...
    protected function getMetadataObj(string $class)
    {
        ...
        return (new DataStructureMetadata($this->cacheDriver))->loadConfig($path);
    }
    ...
    public function getFilePath(string $class)
    {
        if (is_dir($this->path) && is_readable($this->path)) {
            return $this->path . str_replace($this->baseNamespace, '', $class) . '.php';
        }
